...
const CheckoutItem: React.FunctionComponent<Props> = ({ flower, quantity, cartSet, cartDelete }) => {
  ...
  if (quantity === -1) {
    ...
    return (
      <div className="checkout-item">
        <p className="remove-icon" onClick={(): void => cartDelete(flower.id)}>
          &#x4d;
        </p>
        ...
        <div className="details">
          ...
          <div className="flower-quantity">
            <p>Qty:</p>
            <input type="number" value={count} step={1} max={flower.count} min={0} onChange={onChangeQuantity} />
          </div>
        ...
        <p className="total-cost">${((flower.onSale ? flower.salePrice : flower.price) * count).toFixed(2)}</p>
      </div>
    );
  }
};
